...
public class HelloKVLite {
    ...
    public static void main(String[] args) {
        ...
        // C(reate)
        String byeKeyString = "goodbyeKey", byeValueString = "Goodbye, Oracle NoSQL!";
        Key byeKey = Key.createKey(Arrays.asList(byeKeyString));
        Value byeValue = Value.createValue(byeValueString.getBytes());
        store.put(byeKey, byeValue);

        // R(ead)
        String byeResult = new String(store.get(byeKey).getValue().getValue());
        System.out.println(byeKey.toString() + " : " + byeResult);

        // U(pdate)
        byeValueString  = "Goodbye, KVLite!";
        byeValue  = Value.createValue(byeValueString.getBytes());
        store.put(byeKey, byeValue );
        byeResult = new String(store.get(byeKey).getValue().getValue());
        System.out.println(byeKey.toString() + " : " + byeResult);

        // D(elete)
        store.delete(byeKey);
        if (store.get(byeKey) == null) {
            System.out.println("Goodbye key successfully deleted...");
        } else {
            System.out.println("Oops, key not deleted...");
        }
